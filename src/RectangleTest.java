...
public class RectangleTest extends TestCase {
    ...
    public void testIntersect() {
        ...
        left = new Rectangle(rec.getxCoordinate() - 200, 
                             rec.getyCoordinate(), 50, 50);
        assertFalse(rec.intersect(left));
        
        leftTouching = new Rectangle(rec.getxCoordinate() - 200, 
                                     rec.getyCoordinate(), 200, 50);
        assertFalse(rec.intersect(leftTouching));
        
        leftSmallIntersect = new Rectangle(rec.getxCoordinate() - 200, 
                                           rec.getyCoordinate(), 201, 50);
        assertTrue(rec.intersect(leftSmallIntersect));
        
        
        // check rectangle to the right and edge cases
        right = new Rectangle(rec.getxCoordinate() + 300, 
                              rec.getyCoordinate(), 50, 50);
        assertFalse(rec.intersect(right));
        
        rightTouching = new Rectangle(rec.getxCoordinate() - 200, 
                                      rec.getyCoordinate(), 200, 50);
        assertFalse(rec.intersect(rightTouching));

        rightSmallIntersect = new Rectangle(rec.getxCoordinate() - 1, 
                  rec.getyCoordinate(), 200, 50);
        assertTrue(rec.intersect(rightSmallIntersect));
        
        
        // check rectangle to above and edge cases
        above = new Rectangle(rec.getxCoordinate(), 
                              rec.getyCoordinate() - 200, 50, 50);
        assertFalse(rec.intersect(above));
        
        aboveTouching = new Rectangle(rec.getxCoordinate(), 
                                      rec.getyCoordinate() - 200, 200, 200);
        assertFalse(rec.intersect(aboveTouching));

        aboveSmallIntersect = new Rectangle(rec.getxCoordinate(), 
                                            rec.getyCoordinate() - 200, 
                                            200, 201);
        assertTrue(rec.intersect(aboveSmallIntersect));
        
        
        // check rectangle to above and edge cases
        below = new Rectangle(rec.getxCoordinate(), 
                              rec.getyCoordinate() + 300, 50, 50);
        assertFalse(rec.intersect(below));
        
        belowTouching = new Rectangle(rec.getxCoordinate(), 
                                      rec.getyCoordinate() + 200, 200, 200);
        assertFalse(rec.intersect(belowTouching));

        belowSmallIntersect = new Rectangle(rec.getxCoordinate(), 
                                            rec.getyCoordinate() + 199, 
                                            200, 50);
    ...
    public void testEquals() {
        ...
        diffrentXCoord = new Rectangle(rec.getxCoordinate() + 1, 
                                       rec.getyCoordinate(), 
                                       rec.getWidth(), rec.getHeight());
        assertFalse(rec.equals(diffrentXCoord));
        
        diffrentYCoord = new Rectangle(rec.getxCoordinate(), 
                                       rec.getyCoordinate() + 1, 
                                       rec.getWidth(), rec.getHeight());
        assertFalse(rec.equals(diffrentYCoord));
        
        diffrentWidth = new Rectangle(rec.getxCoordinate(), 
                                      rec.getyCoordinate(), 
                                      rec.getWidth() + 1, rec.getHeight());
        assertFalse(rec.equals(diffrentWidth));
        
        diffrentHeight = new Rectangle(rec.getxCoordinate(), 
                                      rec.getyCoordinate(), 
                                      rec.getWidth(), rec.getHeight() + 1);
